...
## 4c. LOOKBOOK — cut-out apparel on true black

Every Shopify product shot has a **pure white (255,255,255) studio background**, which
read as white slabs on the dark site. The lookbook now uses locally processed cut-outs.

- `assets/looks/look-01..07.webp` — transparent, max 1000px, 290 KB for all seven.
- Generated by **flood-filling from the image border**, not by thresholding white.
  This matters: `OLYMPIC T-SHIRT — WHITE` is a white garment on white, and a threshold
  would erase the shirt. Only background connected to the edge is removed.
- Alpha is blurred 1.1px then pulled in (`(a-0.55)/0.35`) to kill the white fringe, and
  edge pixels are un-mixed from white to stop them reading as a halo.
- The lookbook is the **only** section using cut-outs. The product grid still pulls
  straight from the Shopify CDN on `--panel-2` tiles.

If the lookbook lineup changes, `LOOKS` and the files must move together — the JS builds
the src from the index (`look-${n}.webp`), so it does NOT follow the CDN path any more.
`LOOK_GLOW` and `LOOK_TRAIT` are per-position and need updating too.

Treatment: black section + faded chrome grid floor, per-look ambient halo, blurred conic
holographic wash on hover, scanlines, cursor-tracked sheen, RGB-split drop-shadow, slow
float (staggered per tile), and token-style metadata (`Look 01 / 07`, garment name, a
stable FNV-1a hash of the product handle, `1 / 1`). All motion respects
`prefers-reduced-motion`. The hash is decorative — it is **not** on any chain.
